...
elegir los productos para tu lista de bebé y mandarla a tus amigas o familia.</h2>
                       ...
                       <?php 
                     //   $resultss = $mysqli_conn->query("SELECT product_id, product_name, product_image, product_price, category_name FROM products p INNER JOIN  (category c) ON  p.categ_id = c.category_id ");
                       /*   $resultss = $mysqli_conn->query("SELECT product_id, product_name, product_image, product_price, categ_id  FROM  products  ");
                         $categories = $mysqli_conn->query("SELECT category_id, category_name FROM  category ");
                         $cols = '';
                         $rows = '';
                            while($category = $categories->fetch_assoc()) { 
                                //echo $row['category_name'].'<br>'; 		
                               $cols .=  $category['category_name'].'<br>'; 		
                               // echo $row['product_id']; 	 
                               while($row = $resultss->fetch_assoc()) {  
                                  
                                   if($row['categ_id'] == $category['category_id']) {
                                    
                                     $rows .= $row['product_name'].'<br>';
                                   } 
                        ...
                        /* check connection */
                        $resultss = $mysqli_conn->query("SELECT product_id, product_name, product_image, product_price,category_name FROM products p INNER JOIN  (category c) WHERE  p.categ_id = c.category_id ORDER BY c.category_name ,p.product_name; ");
                       ...
                       /*  $cols = '';
                        ...
                        while($row = $resultss->fetch_assoc()) {   
                            if( $cat !== $row["category_name"] ){  
                                ...
                                $rows .= '<p>'.$row["category_name"]." </p> <p>".$row["product_name"].'</p> ';
                                

                                    
                            } else{ 
                                 $rows .= '<p>'.$row["product_name"].'</p> ';    
                            } 
                        }
...
/* 
                        $resulssss = $mysqli_conn->query("SELECT product_id, product_name, product_image, product_price,category_name FROM products p INNER JOIN  (category c) WHERE  p.categ_id = c.category_id ORDER BY c.category_name ,p.product_name; ");
                            ...
                            while($rows = $resulssss->fetch_assoc()){
                                 if($cols != $rows["category_name"]){
                                    echo "<br />".$rows["category_name"]."<br />";
                                    $cols=$rows["category_name"];
                                }
                        ...
                        <?php 
                            while($row1 = $results->fetch_assoc()) {  
                                    if( $cat !== $row1["category_name"] ){  
                                        ...
                                        $row_products .= '
                                            <li class="shopping-categories-item" >'.$row1["category_name"].'</li> 
                                            <li>
